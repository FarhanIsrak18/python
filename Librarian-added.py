...
#     #     closedb()
#     #     home()
#     if e1.get()=='farhan' and e2.get()=='1234':
#         libr()
...
def loginlibr():
    ...
    for row in details:
        if e1.get() in row[0] and e2.get() in row[4]:
            libr()
...
def libr():
    ...
    b5=Button(win, height=2,width=25,text=' Book history ',command=issuedbook)
...
def addusers():
    connectdb()
    q='INSERT INTO Login VALUE("%s","%i","%s","%i","%s")'
    global con,cur
    q1 = 'SELECT userid FROM Login'
    cur.execute(q1)
    details = cur.fetchall()
    for row in details:
        if e2.get() in row:
            messagebox.showerror("User","User Id already exist")
            win.mainloop()
            
    

    cur.execute(q%(e1.get(),int(e2.get()),e3.get(),int(e4.get()),e5.get()))
    con.commit()
    win.destroy()
    # messagebox.showerror("User","User Id already exist")
...
def viewuser():
    ...
    win.geometry("800x300+270+180")
    win.resizable(False,False)
    treeview=Treeview(win,columns=("Name","User ID","Branch","Mobile No","Password"),show='headings')
...
def connectdb():
    ...
    global enter
    if enter==1:
        l='CREATE TABLE IF NOT EXISTS Login(name varchar(20),userid varchar(10),branch varchar(20),mobile int(10),password varchar(50))'
...
def home():
    try:
        ...
        usid=Label(window,text='USERNAME')
